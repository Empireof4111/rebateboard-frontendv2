 ...
 */
import { useMemo, useState, useCallback, type ChangeEvent, type ReactNode } from "react";
import { useNavigate } from "@tanstack/react-router";
import {
  Check, ChevronLeft, ChevronRight, Star, Search, Upload, X,
  ShieldCheck, Bookmark, Layers, Info, AlertCircle, TrendingUp, TrendingDown,
} from "lucide-react";
import { TBI_BRANDS } from "@/lib/tbi-data";
import {
  submitReview, previewTbiImpact,
  type ReviewProviderType, type ReviewProof, type ReviewRatings,
...
const EXPERIENCES = ["Less than 1 month", "1-3 months", "3-6 months", "6-12 months", "More than 1 year"];
...
export function ReviewWizard({ initialProviderType, initialBrandSlug }: Props) {
  ...
  // step 1 state
  const [providerType, setProviderType] = useState<ReviewProviderType | "">(initialProviderType ?? "");
  ...
  const [ratings, setRatings] = useState<ReviewRatings>({
    customerCare: 0, tradingConditions: 0, paymentSpeed: 0,
    userFriendliness: 0, payoutSpeed: 0, overall: 0,
  });
  ...
  const selectedBrand = useMemo(() => TBI_BRANDS.find((b) => b.slug === brandSlug), [brandSlug]);

  const filteredBrands = useMemo(() => {
    const list = providerType ? TBI_BRANDS.filter((b) => b.category === providerType) : TBI_BRANDS;
    return list.filter((b) => b.name.toLowerCase().includes(search.toLowerCase()));
  }, [providerType, search]);

  const tbiDelta = useMemo(
    () => (brandSlug && ratings.overall ? previewTbiImpact(brandSlug, ratings) : 0),
    [brandSlug, ratings],
  );
  ...
  }, [step, providerType, brandSlug, fullName, email, accountSize, customSize, experience, ratings, body, proofs]);
  ...
  const handleSubmit = () => {
    ...
    const finalSize = accountSize === "custom" ? `Custom: ${customSize}` : accountSize || `Custom: ${customSize}`;
    submitReview({
      providerType: selectedBrand.category as ReviewProviderType,
    ...
    toast.success("Review submitted! It's now in moderation.", { description: "You'll earn 50 RR once approved." });
        ...
        <div className="glass rounded-2xl p-5 ring-1 ring-white/10 sm:p-6">
          {step === 1 && (
            <Step1
              ...
              setProviderType={(t) => { setProviderType(t); setBrandSlug(""); }}
              brandSlug={brandSlug}
          ...
          {step === 2 && (
            <Step2
              ...
              accountSize={accountSize} setAccountSize={setAccountSize}
          ...
          {step === 4 && (
            <Step4
              proofs={proofs} onUpload={handleFiles} onRemove={(id) => setProofs((p) => p.filter((x) => x.id !== id))}
              likedMost={likedMost} setLikedMost={setLikedMost}
              likedLeast={likedLeast} setLikedLeast={setLikedLeast}
            />
          )}
        </div>
          ...
          {selectedBrand && (
            <div className="glass rounded-2xl p-4 ring-1 ring-white/10">
              <div className="flex items-center gap-3">
                <div className={`grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br ${selectedBrand.logoColor} text-white text-sm font-bold`}>
                  {selectedBrand.name[0]}
                </div>
                <div className="min-w-0">
                  <div className="truncate text-sm font-bold text-white">{selectedBrand.name}</div>
                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{selectedBrand.category}</div>
                </div>
              </div>
              <div className="mt-3 flex items-center justify-between rounded-xl bg-white/[0.04] p-2.5 ring-1 ring-white/10">
                ...
                <div className="text-base font-bold text-white">{selectedBrand.score.toFixed(1)}<span className="text-[10px] text-muted-foreground">/{selectedBrand.maxScore}</span></div>
              </div>
              {step >= 3 && ratings.overall > 0 && (
                <div className="mt-2 flex items-center justify-between rounded-xl bg-fuchsia-500/10 p-2.5 ring-1 ring-fuchsia-300/30">
                  <div className="inline-flex items-center gap-1 text-[10px] uppercase text-fuchsia-200">
                    {tbiDelta >= 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                    Your impact
                  </div>
                  <div className={`text-sm font-bold ${tbiDelta >= 0 ? "text-emerald-300" : "text-rose-300"}`}>
                    {tbiDelta >= 0 ? "+" : ""}{tbiDelta.toFixed(2)}
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="flex items-center justify-between gap-3 rounded-2xl glass p-4 ring-1 ring-white/10">
        ...
        <div className="flex gap-2">
          ...
          {step < 4 ? (
            ...
            >
              Submit Review <Check className="h-3.5 w-3.5" />
            </button>
          )}
        </div>
      </div>
          ...
          return (
            <div key={label} className="relative z-10 flex flex-col items-center gap-1">
              <div className={
                ...
                (done ? "bg-gradient-to-br from-fuchsia-500 to-violet-600 text-white ring-fuchsia-400/50"
                 : active ? "bg-gradient-to-br from-fuchsia-500 to-violet-600 text-white ring-fuchsia-300/60 shadow-[0_0_18px_rgba(192,132,252,0.6)]"
                 : "bg-white/10 text-muted-foreground ring-white/10")
              }>
                {done ? <Check className="h-3.5 w-3.5" /> : n}
              </div>
              <div className={"text-[10px] font-medium sm:text-[11px] " + (active || done ? "text-white" : "text-muted-foreground")}>
                {label}
              </div>
            </div>
          );
...
function Step1({
  providerType, setProviderType, brandSlug, setBrandSlug, search, setSearch, filteredBrands,
}: {
  ...
  filteredBrands: typeof TBI_BRANDS;
}) {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-white">Choose a Provider Type</h2>
        <p className="mt-1 text-xs text-muted-foreground">Select the type of provider you want to review to get started.</p>
        <select
          value={providerType}
        ...
        >
          <option value="" className="bg-[#1a0b2e]">Select a provider type</option>
          {PROVIDER_TYPES.map((t) => <option key={t} value={t} className="bg-[#1a0b2e]">{t}</option>)}
        </select>
      </div>

      {providerType && (
        <div>
          <h3 className="text-base font-bold text-white">Which {providerType.toLowerCase()} are you reviewing?</h3>
          <p className="mt-1 text-xs text-muted-foreground">Select the {providerType.toLowerCase()} you want to review</p>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 md:grid-cols-3">
            {filteredBrands.map((b) => (
              ...
              >
                <div className={`grid h-9 w-9 place-items-center rounded-lg bg-gradient-to-br ${b.logoColor} text-white text-xs font-bold`}>
                  {b.name[0]}
                </div>
                <div className="min-w-0">
...
function Step2({
  ...
  accountSize, setAccountSize, customSize, setCustomSize,
  experience, setExperience, evalSteps, setEvalSteps,
}: any) {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-white">Your details</h2>
        <p className="mt-1 text-xs text-muted-foreground">Provide your account information for verification.</p>
      </div>

      <Field label="Full Name">
        <input value={fullName} onChange={(e: ChangeEvent<HTMLInputElement>) => setFullName(e.target.value)} className="w-full rounded-xl bg-white/5 px-3 py-2.5 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-fuchsia-400/40" />
      </Field>
      <Field label="Email">
        <input type="email" value={email} onChange={(e: ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)} className="w-full rounded-xl bg-white/5 px-3 py-2.5 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-fuchsia-400/40" />
      </Field>

      <Field label="Account Size">
        <div className="flex flex-wrap gap-2">
          {ACCOUNT_SIZES.map((s) => (
            <ChipBtn key={s} active={accountSize === s} onClick={() => { setAccountSize(s); setCustomSize(""); }}>{s}</ChipBtn>
          ))}
          <ChipBtn active={accountSize === "custom"} onClick={() => setAccountSize("custom")}>Custom size</ChipBtn>
        </div>
        {accountSize === "custom" && (
          <input
      ...
      <Field label="Experience Duration">
        <div className="flex flex-wrap gap-2">
          {EXPERIENCES.map((s) => <ChipBtn key={s} active={experience === s} onClick={() => setExperience(s)}>{s}</ChipBtn>)}
        </div>
      </Field>

      <Field label="Evaluation Steps (optional)">
        <div className="flex flex-wrap gap-2">
          {EVAL_STEPS.map((s) => <ChipBtn key={s} active={evalSteps === s} onClick={() => setEvalSteps(s)}>{s}</ChipBtn>)}
        </div>
      </Field>
    </div>
  );
...
function Step3({ ratings, setRatings, body, setBody }: { ratings: ReviewRatings; setRatings: (r: ReviewRatings) => void; body: string; setBody: (s: string) => void }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">Please provide honest ratings based on your experience.</p>
      </div>
            ...
            <div className="mt-2 flex items-center gap-2">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  aria-label={`${n} stars`}
                >
                  <Star className={`h-5 w-5 ${n <= ratings[key] ? "fill-fuchsia-400 text-fuchsia-400" : "text-white/20"}`} />
                </button>
              ))}
      ...
      <div>
        <div className="mb-1 flex items-center justify-between text-xs">
          ...
          <span className={body.length >= 100 ? "text-emerald-300" : "text-muted-foreground"}>{body.length}/100+</span>
        </div>
        <textarea
          value={body}
...
function Step4({
  proofs, onUpload, onRemove, likedMost, setLikedMost, likedLeast, setLikedLeast,
}: {
  proofs: ReviewProof[];
  onUpload: (files: FileList | null) => void;
  onRemove: (id: string) => void;
  likedMost: string; setLikedMost: (s: string) => void;
  likedLeast: string; setLikedLeast: (s: string) => void;
}) {
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Provide your proof of use, e.g. Payout Certificate, Brokers/Exchangers Live Account Statement.
        </p>
      </div>
      ...
      <Field label={`Attachments (Required) — ${proofs.length}/5`}>
        <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-white/15 bg-white/[0.02] p-6 text-center transition hover:border-fuchsia-300/40 hover:bg-fuchsia-500/5">
          ...
          <div className="text-[11px] text-muted-foreground">Supports JPG, PNG, PDF (Max 20MB each)</div>
        ...
        {proofs.length > 0 && (
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            {proofs.map((p) => (
              <div key={p.id} className="relative overflow-hidden rounded-xl bg-white/[0.04] ring-1 ring-white/10">
                {p.type.startsWith("image/") ? (
                  <img src={p.dataUrl} alt={p.name} className="h-24 w-full object-cover" />
                ) : (
                  <div className="grid h-24 place-items-center bg-fuchsia-500/10 text-[11px] font-semibold text-fuchsia-200">PDF</div>
                )}
                <div className="p-2">
                  <div className="truncate text-[11px] text-white">{p.name}</div>
                  <div className="text-[10px] text-muted-foreground">{(p.size / 1024).toFixed(0)} KB</div>
                </div>
                <button
      ...
      <Field label="3. What do/did you like the most about the item? (optional)">
        <textarea value={likedMost} onChange={(e) => setLikedMost(e.target.value)} rows={3} placeholder="Summary here…" className="w-full rounded-xl bg-white/5 p-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-fuchsia-400/40" />
      </Field>
      <Field label="4. What do/did you like the least about the item? (optional)">
        <textarea value={likedLeast} onChange={(e) => setLikedLeast(e.target.value)} rows={3} placeholder="Summary here…" className="w-full rounded-xl bg-white/5 p-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-fuchsia-400/40" />
      </Field>
    </div>
  );
}
...
function SidebarNote({ step }: { step: 1 | 2 | 3 | 4 }) {
  if (step === 1) {
    return (
      <NoteCard icon={<AlertCircle className="h-4 w-4 text-amber-300" />} title="Important Notes" tone="amber">
  ...
  if (step === 2) {
    return (
      <NoteCard icon={<ShieldCheck className="h-4 w-4 text-cyan-300" />} title="Verification Requirements" tone="cyan">
  ...
  if (step === 3) {
    return (
      <NoteCard icon={<Layers className="h-4 w-4 text-fuchsia-300" />} title="Rating Guidelines" tone="fuchsia">
  ...
  return (
    <NoteCard icon={<Bookmark className="h-4 w-4 text-emerald-300" />} title="Submission Guidelines" tone="emerald">
...
function NoteCard({ icon, title, children, tone }: { icon: ReactNode; title: string; children: ReactNode; tone: "amber" | "cyan" | "fuchsia" | "emerald" }) {
  const toneRing = {
