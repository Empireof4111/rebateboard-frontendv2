...
export function Logo({
  ...
  const mark = (
    <span className={`inline-flex ${heightClass} aspect-square shrink-0 items-center justify-center`} aria-hidden>
      <img
        src={logoMarkSrc}
        alt=""
        className="h-[172%] w-[172%] max-w-none object-contain"
      />
    </span>
  );
  ...
  return (
    ...
    >
      {mark}
      <span className="ml-px whitespace-nowrap text-[1.22rem] font-semibold leading-none text-white sm:text-[1.34rem]">
        RebateBoard
      </span>
    </span>
  );
