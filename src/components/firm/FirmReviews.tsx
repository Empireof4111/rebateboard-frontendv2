    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {firmSlug ? (
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-fuchsia-500 to-violet-600 px-4 py-1.5 text-xs font-bold text-white shadow-[0_0_18px_rgba(192,132,252,0.5)] transition hover:scale-[1.02]"
            >
      ...
      {stats.total === 0 ? (
        <ReviewState
          ...
          action={firmSlug ? (
            <Link to="/review" search={{ itemSlug: firmSlug }} className="rounded-full bg-gradient-to-r from-fuchsia-500 to-violet-600 px-4 py-2 text-xs font-semibold text-white">
              Write a Review
            </Link>
          ) : null}
        />
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            {review.verifiedTrader || review.proofs.length > 0 ? (
              <BadgeCheck className="absolute -right-1 -top-1 h-4 w-4 rounded-full bg-[#1a0b2e] text-emerald-300" />
            ) : null}
          </div>
          <div>
