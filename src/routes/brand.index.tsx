...
import { TrendingUp, Star, ShieldAlert, MessageSquare, ExternalLink, Sparkles, AlertTriangle, ArrowRight } from "lucide-react";
...
function StatCard({ label, value, delta, tone = "neutral", icon: Icon }: { label: string; value: string; delta?: string; tone?: "up" | "down" | "neutral"; icon: any }) {
  ...
  return (
    <div className="glass rounded-2xl p-4 ring-1 ring-white/10">
      <div className="flex items-center gap-2 text-fuchsia-300"><Icon className="h-4 w-4" /></div>
...
function BrandOverview() {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-fuchsia-500/10 via-violet-600/5 to-transparent p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-widest text-fuchsia-300/80">Brand portal</div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_1fr]">
        <div className="glass rounded-2xl p-5 ring-1 ring-white/10">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link to="/brand/reviews" className="text-[11px] text-fuchsia-300 hover:underline inline-flex items-center gap-1">Inbox <ArrowRight className="h-3 w-3" /></Link>
          </div>
          <div className="space-y-2">
            {allReviews.slice(0, 5).map((r) => (
              <div key={r.id} className="flex items-start gap-3 rounded-xl bg-white/[0.04] p-3 ring-1 ring-white/5">
                <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-gradient-to-br from-fuchsia-400 to-violet-600 text-[10px] font-bold text-white">{r.userName.slice(0, 2).toUpperCase()}</div>
