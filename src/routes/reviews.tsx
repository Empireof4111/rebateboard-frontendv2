...
import { BadgeCheck, ChevronRight, MessageSquare, Search, ShieldCheck, Star } from "lucide-react";
      ...
      <main className="container-app py-8 sm:py-10">
        <section className="glass rounded-2xl p-6 ring-1 ring-white/10 sm:p-8">
          <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
            <div>
              <div className="text-xs uppercase tracking-[0.16em] text-fuchsia-300">Trust Engine</div>
        ...
        <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {CATEGORIES.map((item) => {
            ...
            return (
              <button key={item.key} type="button" onClick={() => setCategory(item.key)} className="glass flex items-center justify-between rounded-xl p-4 text-left ring-1 ring-white/10 transition hover:-translate-y-0.5 hover:ring-fuchsia-300/35">
                <div>
                  <div className="text-sm font-semibold text-white">{item.label}</div>
                  <div className="mt-0.5 text-[10px] text-muted-foreground">{item.description}</div>
                  <div className="mt-2 text-[10px] font-semibold text-fuchsia-200">{count} reviews</div>
                </div>
        ...
        <section className="glass mt-6 rounded-2xl p-4 ring-1 ring-white/10">
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => setVerifiedOnly((value) => !value)} className={`rounded-xl px-3 py-2 text-xs font-semibold ring-1 transition ${verifiedOnly ? "bg-fuchsia-500/15 text-white ring-fuchsia-300/40" : "bg-white/5 text-muted-foreground ring-white/10"}`}>
              Verified only
            </button>
          </div>
        </section>
...
function ReviewCard({ review }: { review: ReviewRecord }) {
  return (
    <article className="glass rounded-2xl p-4 ring-1 ring-white/10 transition hover:-translate-y-0.5 hover:ring-fuchsia-300/30">
      <div className="flex items-start gap-3">
        <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-white/5 ring-1 ring-white/10">
          {review.brandLogo ? <img src={review.brandLogo} alt="" className="h-full w-full object-cover" loading="lazy" /> : <span className="font-bold text-white">{review.brandName[0]}</span>}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <Link to="/firm/$firmId" params={{ firmId: review.brandSlug }} className="text-sm font-bold text-white hover:text-fuchsia-200">{review.brandName}</Link>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px] text-muted-foreground">
            ...
            {review.verifiedPayout ? <Badge label="Proof-backed" /> : null}
          </div>
        </div>
      </div>
...
function Stars({ value }: { value: number }) {
  return <div className="flex">{Array.from({ length: 5 }).map((_, index) => <Star key={index} className={`h-3.5 w-3.5 ${index < Math.round(value) ? "fill-fuchsia-400 text-fuchsia-400" : "text-white/15"}`} />)}</div>;
}
