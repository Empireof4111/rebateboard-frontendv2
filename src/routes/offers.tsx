...
export const Route = createFileRoute("/offers")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse exclusive prop firm, broker, exchange and trading-tool promos. Verified discount codes, free accounts and limited-time deals." },
      { property: "og:title", content: "RebateBoard — Exclusive Offers" },
      { property: "og:description", content: "Hand-picked promos and discount codes across prop firms, brokers, exchanges and tools." },
    ],
  }),
...
function SectionHeader({ icon: Icon, title, subtitle, accent }: { icon: typeof Flame; title: string; subtitle: string; accent: string }) {
  return (
...
function PublicOffers() {
  ...
  const exclusive = useMemo(() => live.filter((o) => o.tags?.includes("exclusive") || o.pinned).slice(0, 6), [live]);
  const latest = useMemo(() => [...live].sort((a, b) => (b.createdAt ?? "").localeCompare(a.createdAt ?? "")).slice(0, 6), [live]);
  const endingSoon = useMemo(() => live.filter((o) => o.tags?.includes("limited") || /May|Jun/.test(o.expires)).slice(0, 6), [live]);
      ...
      <main className="mx-auto max-w-7xl space-y-12 px-4 py-10 sm:px-6 lg:px-8">
        ...
        <header className="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-fuchsia-500/20 via-violet-600/15 to-cyan-500/15 p-8 sm:p-12">
          ...
          <p className="mt-2 max-w-2xl text-sm text-white/70 sm:text-base">
            Hand-picked discounts on prop firm challenges, broker bonuses, exchange fee rebates, and trading tools — all in one place.
          </p>

          <div className="mt-6 flex max-w-xl items-center gap-2 rounded-full border border-white/15 bg-black/30 px-4 py-2.5">
            <Search className="h-4 w-4 text-white/50" />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search FTMO, Bybit, free account…" className="w-full bg-transparent text-sm outline-none placeholder:text-white/40" />
          </div>
        </header>
        ...
        {exclusive.length > 0 && (
          <section>
            <SectionHeader icon={Flame} title="Exclusive offers" subtitle="Negotiated by RebateBoard — only available here." accent="bg-gradient-to-br from-fuchsia-500 to-pink-600" />
        ...
        {/* Latest */}
        <section>
          <SectionHeader icon={Sparkles} title="Latest offers" subtitle="Freshly added this week." accent="bg-gradient-to-br from-emerald-500 to-cyan-600" />
          <div className="grid gap-3 lg:grid-cols-2">
        ...
        {endingSoon.length > 0 && (
          <section>
            <SectionHeader icon={Clock} title="Ending soon" subtitle="Last chance — these promos expire in days." accent="bg-gradient-to-br from-amber-500 to-rose-600" />
            <div className="grid gap-3 lg:grid-cols-2">
        ...
        {/* By Category */}
        <section>
          <SectionHeader icon={LayoutGrid} title="Browse by category" subtitle="Filter the full catalogue across every brand we list." accent="bg-gradient-to-br from-sky-500 to-indigo-600" />
          ...
          <div className="mb-4 flex flex-wrap gap-1.5">
            {CATEGORIES.map((c) => (
              <button key={c} onClick={() => setFilter(c)}
                ...
                {c} <span className="ml-1 text-[10px] text-white/40">{c === "All" ? live.length : live.filter((o) => o.category === c).length}</span>
              </button>
            ))}
          </div>
