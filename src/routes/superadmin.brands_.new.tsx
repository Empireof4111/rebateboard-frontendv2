...
const inputCls = "w-full rounded-lg bg-white/5 px-3 py-2 text-sm text-white ring-1 ring-white/10 focus:ring-violet-400/40 focus:outline-none placeholder:text-muted-foreground";
...
function NewBrandPage() {
  ...
  const [primaryColor, setPrimaryColor] = useState("#5a22f1");
  ...
  function hydrateFromBrand(brand: AdminBrandRecord) {
    ...
    setPrimaryColor(brand.primaryColor ?? "#5a22f1");
    ...
    return (
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button
          ...
          disabled={saving}
          className="rounded-full rb-gradient-primary px-4 py-1.5 text-xs font-bold text-white disabled:opacity-60"
        >
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <button
              className="inline-flex items-center gap-1.5 rounded-full rb-gradient-primary px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-60"
                ...
                return (
                  <button
                    ...
                    className={`group flex items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs transition ${
                      active ? "bg-gradient-to-r from-violet-500/20 to-violet-600/20 text-white ring-1 ring-violet-400/30" : "text-muted-foreground hover:bg-white/5 hover:text-white"
                    }`}
                  >
                    <span className={`grid h-5 w-5 place-items-center rounded-md text-[10px] font-bold ${active ? "bg-violet-500/30 text-violet-100" : done ? "bg-emerald-500/20 text-emerald-300" : "bg-white/10 text-muted-foreground"}`}>
        ...
        <div className="space-y-4">
          ...
          {section === "category" && (
            <Panel title="Brand category">
              ...
              <p className="mt-3 text-[11px] text-muted-foreground">Selected: <span className="font-bold text-violet-300">{category}</span></p>
              <SectionActions />
            </Panel>
          )}
          ...
          {section === "challenges" && isProp && (
            ...
            >
                  ...
                  <div key={c.id} className="rounded-xl bg-white/5 p-3 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="rounded-full bg-violet-500/15 px-2 py-0.5 text-[10px] font-bold text-violet-300 ring-1 ring-violet-400/30">#{i + 1}</span>
              ...
              <button type="button" onClick={addChallenge} className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-white/10 bg-white/5 px-3 py-2.5 text-xs font-semibold text-muted-foreground hover:border-violet-400/30 hover:text-white">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Logo (SVG / PNG)" span={2}>
                  <label className="block">
                    ...
                    />
                    <div className="relative flex h-32 cursor-pointer items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-white/10 bg-white/5 text-xs text-muted-foreground hover:border-violet-400/30">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="KYB / license document" span={2}>
                  <label className="block">
                    ...
                    />
                    <div className="flex h-20 cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-white/10 bg-white/5 text-xs text-muted-foreground hover:border-violet-400/30">
              ...
              <div className="space-y-3 text-sm text-white">
                ...
                <div className="pt-2">
                  <button
                    onClick={() => void persistBrand("published")}
                    disabled={saving}
                    className="w-full rounded-xl rb-gradient-primary px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"
                  >
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
      <label className="flex cursor-pointer items-center justify-between gap-3">
        ...
        <button
          ...
          className={`relative h-5 w-9 shrink-0 rounded-full transition ${checked ? "rb-gradient-primary" : "bg-white/10"}`}
        >
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        type="button"
        onClick={onAdd}
        className="inline-flex w-full items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-white/10 bg-white/5 px-3 py-2.5 text-xs font-semibold text-muted-foreground hover:border-violet-400/30 hover:text-white"
      >
...
function StepListPanel({
  ...
  return (
    <Panel
      ...
      action={
        <button onClick={ctl.add} className="inline-flex items-center gap-1.5 rounded-full rb-gradient-primary px-3 py-1.5 text-[11px] font-bold text-white">
        ...
        {ctl.list.map((s, i) => (
          <div key={s.id} className="rounded-xl bg-white/5 p-3 ring-1 ring-white/10">
            <div className="mb-2 flex items-center gap-2">
              <span className="grid h-6 w-6 place-items-center rounded-full rb-gradient-primary text-[10px] font-bold text-white">{i + 1}</span>
              <input
                className={inputCls}
        ...
        {ctl.list.map((rule, index) => (
          <div key={rule.id} className="rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10 transition hover:bg-white/[0.06]">
            <div className="mb-3 flex items-start gap-3">
              <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full rb-gradient-primary text-[10px] font-bold text-white">
                {index + 1}
              </span>
