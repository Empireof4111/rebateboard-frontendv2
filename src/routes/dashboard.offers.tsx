...
function DashboardOffers() {
  ...
  const exclusive = useMemo(
    () => live.filter((o) => o.tags?.includes("exclusive") || o.pinned).slice(0, 4),
    [live],
  );
  const latest = useMemo(
    () =>
      [...live].sort((a, b) => (b.createdAt ?? "").localeCompare(a.createdAt ?? "")).slice(0, 4),
    [live],
  );
  const endingSoon = useMemo(
    () => live.filter((o) => o.tags?.includes("limited")).slice(0, 4),
    [live],
  );
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Verified discounts across every brand we list.
          </p>
        </div>
        <Link
          to="/offers"
          className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-4 py-2 text-xs font-semibold text-white hover:bg-white/15"
        >
