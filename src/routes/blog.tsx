...
import { Search, Clock, User, ArrowRight, TrendingUp, Filter, BookOpen, Calendar } from "lucide-react";
...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Guides, comparisons and industry reports on prop firms, brokers, payouts and trader strategy." },
      { property: "og:title", content: "Blog & Insights — RebateBoard" },
      { property: "og:description", content: "Guides, comparisons and industry reports for serious traders." },
    ],
  }),
  component: BlogIndex,
});

const TAGS = ["All", "Guide", "Comparison", "Brand spotlight", "Trader 101", "Industry", "Partner news"];
  ...
  return (
    <div className="min-h-screen bg-[#0d0420] text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="mx-auto max-w-7xl px-4 py-14 sm:py-20">
          ...
          <h1 className="mt-3 max-w-3xl text-4xl font-bold leading-tight sm:text-5xl">
            Insights for traders who care about the <span className="bg-gradient-to-r from-fuchsia-400 to-violet-400 bg-clip-text text-transparent">truth behind the brand</span>.
          </h1>
          <p className="mt-4 max-w-2xl text-base text-white/70">
            Honest reporting on prop firms, brokers, payouts and trader strategy — no affiliate spin, no recycled press releases.
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {TAGS.map((t) => {
              ...
              return (
                ...
                >
                  {t}
                  <span className={`rounded-full px-1.5 text-[10px] ${active ? "bg-white/25" : "bg-white/10"}`}>{count}</span>
                </button>
              );
            })}
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        {filtered.length === 0 ? (
          <EmptyState onReset={() => { setQ(""); setTag("All"); }} />
        ) : (
          <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
            ...
            <aside className="space-y-6">
                ...
                <ol className="mt-4 space-y-4">
                  {trending.map((p, i) => (
                    <li key={p.id} className="flex gap-3">
                      ...
                      <Link to="/articles/$id" params={{ id: p.id }} className="group flex-1">
                        <h3 className="text-sm font-semibold leading-snug text-white group-hover:text-fuchsia-300">{p.title}</h3>
                        <p className="mt-1 text-[11px] text-white/50">{p.author} · {p.views} views</p>
                      </Link>
                    </li>
                  ))}
                </ol>
              ...
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-fuchsia-600/30 to-violet-700/30 p-5 ring-1 ring-fuchsia-400/30">
                ...
                <p className="mt-2 text-xs text-white/70">The single email serious traders read. Payout shifts, firm changes, and one strong setup. No fluff.</p>
                <form className="mt-4 flex gap-2" onSubmit={(e) => e.preventDefault()}>
                  <input placeholder="you@trader.com" className="flex-1 rounded-xl bg-black/30 px-3 py-2 text-xs outline-none ring-1 ring-white/10 placeholder:text-white/40" />
                  <button className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-[#0d0420]">Join</button>
                </form>
              </div>
              ...
              <div className="rounded-2xl bg-white/[0.04] p-5 ring-1 ring-white/10">
                ...
                <ul className="mt-3 space-y-2 text-xs">
                  {TAGS.filter((t) => t !== "All").map((t) => (
                    <li key={t}>
                      <button onClick={() => setTag(t)} className="flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-white/70 hover:bg-white/5 hover:text-white">
                        <span>{t}</span>
    ...
    >
      <div className="grid md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center p-6 sm:p-8">
          ...
          <h2 className="mt-3 text-2xl font-bold leading-tight group-hover:text-fuchsia-200 sm:text-3xl">{post.title}</h2>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="text-base font-bold leading-snug group-hover:text-fuchsia-200">{post.title}</h3>
...
function Meta({ post, className = "" }: { post: BlogPost; className?: string }) {
  return (
    <div className={`flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-white/50 ${className}`}>
      <span className="inline-flex items-center gap-1"><User className="h-3 w-3" />{post.author}</span>
      <span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" />{post.time}</span>
...
function EmptyState({ onReset }: { onReset: () => void }) {
  return (
    <div className="rounded-3xl border border-dashed border-white/15 p-16 text-center">
      <h3 className="text-lg font-bold">No articles match your filters</h3>
      <p className="mt-2 text-sm text-white/60">Try a different search or category.</p>
      <button onClick={onReset} className="mt-5 rounded-full bg-white/10 px-4 py-2 text-xs font-semibold hover:bg-white/20">
        Reset filters
      </button>
    <SiteFooter />
    </div>
