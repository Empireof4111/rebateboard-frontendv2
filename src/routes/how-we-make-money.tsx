      ...
      <main className="container-app py-8 sm:py-10">
        <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-violet-900/30 via-fuchsia-900/10 to-[#0b0418] p-8 md:p-14">
          ...
          <div className="relative max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-fuchsia-200">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              {["Transparency", "Verification", "Cashback", "Community"].map((item, index) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                  <div className="grid h-9 w-9 place-items-center rounded-xl bg-fuchsia-500/15 text-fuchsia-200 ring-1 ring-fuchsia-300/20">{index + 1}</div>
                  <div className="mt-3 text-sm font-bold">{item}</div>
                  <div className="mt-2 h-1 rounded-full bg-white/10">
                    <div className="h-full rounded-full bg-gradient-to-r from-fuchsia-400 to-violet-400" style={{ width: `${55 + index * 12}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="mt-10">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {revenueSources.map((source) => (
              <div key={source.title} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition hover:-translate-y-0.5 hover:border-fuchsia-300/30">
                <source.icon className="h-5 w-5 text-fuchsia-200" />
            ...
            <div className="grid gap-3 sm:grid-cols-4">
              {[
                ...
                <div key={step.label} className="relative rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-center">
                  <step.icon className="mx-auto h-5 w-5 text-fuchsia-200" />
                  <div className="mt-2 text-xs font-bold">{step.label}</div>
                  {index < 3 && <ArrowRight className="absolute -right-4 top-1/2 hidden h-4 w-4 -translate-y-1/2 text-fuchsia-200/70 sm:block" />}
                </div>
              ))}
            </div>
        ...
        <section className="mt-10 rounded-3xl border border-fuchsia-300/15 bg-gradient-to-br from-fuchsia-500/10 via-white/[0.03] to-violet-600/10 p-8 md:p-10">
          <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-fuchsia-200/80">Our commitment</div>
            ...
            {faqs.map((faq) => (
              <details key={faq.q} className="group rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <summary className="cursor-pointer list-none text-sm font-bold text-white">
                  <span>{faq.q}</span>
                  <span className="float-right text-fuchsia-200 transition group-open:rotate-45">+</span>
                </summary>
        ...
        <section className="mt-10 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-violet-600/20 via-fuchsia-500/10 to-white/[0.03] p-8 text-center md:p-12">
          <LockKeyhole className="mx-auto h-7 w-7 text-fuchsia-200" />
...
function SectionHeader({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div>
      <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-fuchsia-200/80">{eyebrow}</div>
